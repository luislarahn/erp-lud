...
export default function ProformaPage() {
  ...
  const [detalle, setDetalle] = useState<DetalleProforma[]>([])
  ...
  async function cargarProforma() {
    ...
    try {
      ...
      setProforma(proformaData as Proforma)
      setDetalle((detalleData || []) as DetalleProforma[])
    } catch (error: any) {
      console.log('Error al cargar factura proforma:', error)
  ...
  if (mensaje || !proforma) {
    return (
      ...
      >
        ...
        >
          {mensaje || 'No se encontró la factura proforma.'}
        </div>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Documento no fiscal. Esta factura proforma no sustituye factura.
            </p>
          </div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            >
              <div>
                <strong>N.º:</strong> {proforma.numero_proforma}
              </div>
              <div>
                <strong>Fecha:</strong> {fechaLocal(proforma.fecha_proforma)}
              </div>
              <div>
                <strong>Estado:</strong> {proforma.estado}
              </div>
            </div>
          </div>
        </header>
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: '12px', color: '#3F4A56', lineHeight: 1.7 }}>
              <div>
                <strong>Nombre:</strong> {proforma.nombre_cliente}
              </div>
              <div>
                <strong>RTN:</strong> {proforma.rtn || '-'}
              </div>
              <div>
                <strong>Dirección:</strong> {proforma.direccion || '-'}
              </div>
              <div>
                <strong>Teléfono:</strong> {proforma.telefono || '-'}
              </div>
              <div>
                <strong>Correo:</strong> {proforma.correo || '-'}
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div style={{ fontSize: '12px', color: '#3F4A56', lineHeight: 1.7 }}>
              <div>
                <strong>Tipo:</strong> Factura Proforma
              </div>
              <div>
                <strong>Número interno:</strong> {proforma.numero_proforma}
              </div>
              <div>
        ...
        >
          ...
          >
            ...
            <p style={{ marginTop: '6px' }}>
              {proforma.observaciones ||
                'Esta factura proforma es un documento comercial preliminar. Los precios y condiciones pueden estar sujetos a cambios según disponibilidad, alcance final y aprobación correspondiente.'}
            </p>
          </div>
          ...
          >
            <FilaTotal label="Importe exonerado" valor={proforma.importe_exonerado} />
            <FilaTotal label="Importe exento" valor={proforma.importe_exento} />
            <FilaTotal label="Importe gravado 15%" valor={proforma.importe_gravado_15} />
            <FilaTotal label="Importe gravado 18%" valor={proforma.importe_gravado_18} />
            <FilaTotal label="ISV 15%" valor={proforma.isv_15} />
            <FilaTotal label="ISV 18%" valor={proforma.isv_18} />
            <FilaTotal label="Subtotal" valor={proforma.subtotal} />
            <FilaTotal label="Impuesto total" valor={proforma.impuesto_total} />
            <FilaTotal
              label="Total proforma"
              valor={proforma.total_proforma}
              destacado
            />
          </div>
        </section>
        ...
        >
          Factura Proforma generada desde ERP LUD. Este documento no tiene validez fiscal.
        </footer>
      </main>
    </div>
  )
